...
MOCK_LLM_EOF
  ...
  echo ""
  echo -e "  ${BOLD}2. Demo vs production escalation:${RESET}"
  echo "     This demo uses a low threshold (1 CRITICAL event) for quick"
  echo "     demonstration. In production, tool stripping alone should NOT"
  echo "     quarantine an agent — agents routinely include tools in requests"
  echo "     that the LLM may never call. A stripped tool in the request path"
  echo "     is preventive, not evidence of malicious behavior."
  echo "     Real escalation should be based on repeated LLM response-path"
  echo "     violations (tool calls the LLM actually attempted), using higher"
  echo "     thresholds and longer time windows."
  echo ""
  echo -e "  ${BOLD}3. AgentGateway ImmediateResponse (not yet available):${RESET}"
  ...
  echo ""
  echo -e "  ${BOLD}4. NetworkPolicy enforcement (graduated_containment track):${RESET}"
